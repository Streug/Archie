...
class PDFReviewer:
    ...
    def _load_existing_csv(self):
        ...
        if os.path.exists(cfg.CSVBESTANDSNAAM1):
            try:
                with open(cfg.CSVBESTANDSNAAM1, newline="", encoding="utf-8") as f:
                ...
                self._set_status(f"✓ {len(self.all_results)} bestaande rijen geladen")
            except Exception:
                pass
    ...
    def _save_and_next(self):
        ...
        if existing:
            self.all_results = [r for r in self.all_results if r["bestand"] != filename]
        ...
        if self.current_index < len(self.pdf_files) - 1:
            ...
            messagebox.showinfo(
                ...
                f"Resultaten staan in: {cfg.CSVBESTANDSNAAM2}",
            )
    ...
    def _save_csv(self):
        ...
        messagebox.showinfo(
            "Opgeslagen",
            f"CSV opgeslagen als:\n{os.path.abspath(cfg.CSVBESTANDSNAAM2)}",
        )

    def _write_csv(self):
        fieldnames = ["bestand"] + cfg.KENMERKEN
        with open(cfg.CSVBESTANDSNAAM2, "w", newline="", encoding="utf-8") as f:
